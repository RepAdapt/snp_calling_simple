...
CHROM=$(sed -n "${SLURM_ARRAY_TASK_ID}p" chromosomes.txt) ### list of chromosomes (can be found in the FASTA index file of reference genome -- .fai file). This species has 14, that s why array number is 14. We parallelize by crhomosome. if you had a very fragmented reference with like 1000s of scaffolds, rather than sending 1000 jobs, you can feed a list of chromosomes to the command. so you can for example split 1000 scaffolds into 10 lists of 100 scaffolds names each, and feed those 10 lists to the command using a job array of size 10 (1-10) -- one list of scaffolds per job
